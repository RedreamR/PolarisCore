...
public class InGame_DeclareWar extends Menu {
    ...
    public InGame_DeclareWar(int nCivID) {
        ...
        int iconWidth = (int)Math.ceil((float)ImageManager.getImage(Images.gold).getWidth() * 1.5F);
        ...
        if (alliesLeft.size() > 0 || alliesRight.size() > 0) {
            ...
            for(int i = 0; i < alliesLeft.size(); ++i) {
                menuElements.add(new Text_StaticBG_ID_FlagCiv_GreenRed(Game.getCiv(alliesLeft.get(i)).getCivName(), CFG.FONT_REGULAR_SMALL, CFG.PADDING * 2, paddingLeft, buttonY, alliesWidth, buttonH, alliesLeft.get(i)) {
                    public void actionElement() {
            ...
            for(int i = 0; i < alliesRight.size(); ++i) {
                menuElements.add(new Text_StaticBG_ID_FlagCiv(Game.getCiv(alliesRight.get(i)).getCivName(), CFG.FONT_REGULAR_SMALL, CFG.PADDING * 2, paddingLeft + CFG.PADDING + alliesWidth, buttonY, alliesWidth, buttonH, alliesRight.get(i)) {
                    public void actionElement() {
